...
GuideFrame::GuideFrame(GUI_App *pGUI, long style)
    : DPIDialog((wxWindow *) (pGUI->mainframe), wxID_ANY, "XDesktop", wxDefaultPosition, wxDefaultSize, style),
	m_appconfig_new()
{
        ...
        if (sVendor == "" || sType == "")
        {
            if (jLocal.contains("inherits")) {
                ...
                if (boost::filesystem::exists(inherits_path))
                    ...
                    BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << " inherits File Not Exist: " << PathSanitizer::sanitize(inherits_path.string());
                    return -1;
                }
            } else {
...
int GuideFrame::LoadProfileFamily(std::string strVendor, std::string strFilePath)
{
    ...
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << boost::format(",  vendor path %1%.") % PathSanitizer::sanitize(vendor_dir.string());
    try {
...
bool GuideFrame::LoadFile(std::string jPath, std::string &sContent)
{
    try {
        ...
        buffer << t.rdbuf();
        sContent=buffer.str();
        BOOST_LOG_TRIVIAL(trace) << __FUNCTION__ << boost::format(", load %1% into buffer")% PathSanitizer::sanitize(jPath);
    }
    catch (std::exception &e)
    {
