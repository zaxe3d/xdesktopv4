    ...
    : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxSize(42 * wxGetApp().em_unit(), -1)), p(new priv(parent, side_tools))
{
    ...
    // add printer
    {
        ...
        p->m_printer_setting = new ScalableButton(p->m_panel_printer_title, wxID_ANY, "zaxe_settings_blue");
    ...
    StateColor flush_bd_col(std::pair<wxColour, int>(wxColour(0, 154, 222), StateColor::Pressed),
                            std::pair<wxColour, int>(wxColour(0, 154, 222), StateColor::Hovered),
    ...
    ScalableButton* edit_btn = new ScalableButton(p->m_panel_filament_content, wxID_ANY, "zaxe_export_blue");
    ...
    p->sizer_filaments->Add(combo_and_btn_sizer, 0, wxEXPAND | wxLEFT | wxRIGHT, FromDIP(15));

    //bSizer_filament_content->Add(p->sizer_filaments, 1, wxALIGN_CENTER | wxALL);
    wxSizer *sizer_filaments2 = new wxBoxSizer(wxVERTICAL);
    sizer_filaments2->AddSpacer(FromDIP(5));
    ...
    //p->sizer_params->Add(FromDIP(3), 0, 0, 0, 0);
    p->sizer_params->Add(p->m_search_bar, 0, wxLEFT | wxRIGHT | wxEXPAND, FromDIP(20));
    ...
    p->machine_manager->scrolledArea()->Bind(wxEVT_BUTTON, [this](auto& e) {
        ...
        if (user_data) {
            ...
            std::string model_suffix = (boost::format("@%1%") % boost::to_upper_copy(_nm->attr->device_model)).str();
            ...
            for (int i = 0; i < 2; ++i) {
                                           ...
                                           [&](const auto& p) {
                                               std::string preset_name =
                                                   wxGetApp().preset_bundle->get_preset_name_by_alias(Preset::TYPE_PRINTER,
                                                                                                      Preset::remove_suffix_modified(
                                                                                                          p.ToUTF8().data()));
                                               return case_insensitive_compare(preset_name, current_printer_preset_name);
                                           });
                    it != printers.end()) {
                ...
                if (!_nm->states->filamentPresent || case_insensitive_compare(_nm->attr->material, "custom")) {
                    is_filament_found   = true;
                    hide_preset_details = false;
                } else {
                    const auto& filaments                    = p->combos_filament.front()->GetValues();
                    std::string current_filament_preset_name = wxGetApp().preset_bundle->get_preset_name_by_material_label(
                        _nm->attr->material, model_suffix);
                    if (auto it = std::find_if(filaments.begin(), filaments.end(),
                                               [&](const auto& f) {
                                                   ...
                                                   return case_insensitive_compare(preset_name, current_filament_preset_name);
                                               });
                        it != filaments.end()) {
                ...
                if (is_printer_found && is_filament_found) {
                    ...
                    if (!is_printer_found) {
                        ...
                        std::string current_filament_preset_name =
                            wxGetApp().preset_bundle->get_preset_name_by_material_label(_nm->attr->material, model_suffix);
            ...
            auto ask_for_conf_wizard = [&](auto _question, auto _page) {
                RichMessageDialog dialog(GetParent(), _question, _L("XDesktop: Confirmation"), wxICON_QUESTION | wxYES_NO);
                dialog.SetYesNoLabels(_L("Yes"), _L("No"));
                int res = dialog.ShowModal();
                if (res == wxID_YES) {
                    wxGetApp().run_wizard(ConfigWizard::RR_USER, _page);
                    on_select_preset();
                }
            };
...
void Sidebar::init_filament_combo(PlaterPresetComboBox **combo, const int filament_idx)
{
    ...
    */
    ...
    this->p->sizer_filaments->Add(combo_and_btn_sizer, 0, wxEXPAND | wxLEFT | wxRIGHT, FromDIP(15));
}
...
void Sidebar::remove_unused_filament_combos(const size_t current_extruder_count)
{
    ...
    while (p->combos_filament.size() > current_extruder_count) {
        ...
        this->p->sizer_filaments->Remove(last);
...
void Sidebar::sync_ams_list()
{
    ...
    if (unknowns > 0) {
        MessageDialog dlg(this,
            _L("There are some unknown filaments mapped to generic preset. Please update XDesktop or restart XDesktop to check if there is an update to system presets."),
            _L("Sync filaments with AMS"), wxOK);
        dlg.ShowModal();
    }
...
void Plater::priv::bring_instance_forward() const
{
    ...
    BOOST_LOG_TRIVIAL(debug) << "XDesktop window going forward";
            ...
            http.on_progress([&percent, &cont, &msg, &filesize, &size_limit](Http::Progress progress, bool& cancel) {
                ...
                .on_error([&msg, &cont, &retry_count, max_retries](std::string body, std::string error, unsigned http_status) {
                    ...
                    if (retry_count == max_retries) {
                        msg = _L("Importing to XDesktop failed. Please download the file and manually import it.");
                        cont = false;
                    }
                })
...
void Plater::show_object_info()
{
    ...
    if (non_manifold_edges > 0) {
        info_manifold += into_u8("\n" + _L("Tips:") + "\n" +_L("\"Fix Model\" feature is currently only on Windows. Please repair the model on XDesktop(windows) or CAD softwares."));
    }
    #endif //APPLE & LINUX
